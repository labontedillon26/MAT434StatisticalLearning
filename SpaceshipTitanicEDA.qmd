...
author: 
  - name: Adam Gilbert, Dillon Labonte
...
Let's build and tune a support vector machine model using radial basis functions. The metric we are interested in is recall, since we want to accurately be able to flag a passenger that would likely be transported to an alternate dimension.
...
svm_rbf_tune_results <- svm_rbf_wf %>%
  tune_bayes(
    grid = svm_rbf_grid,
    resamples = train_folds,
...
Let's look at the 10 best models from this tuning process.

```{r}

#save(svm_rbf_tune_results, svm_rbf_wf, file = "svm_rbf_tune_results.RData")
load("svm_rbf_tune_results.RData")
...
Let's fit the best model to our training data, and see what our recall metric is.

```{r}

best_svm_rbf <- svm_rbf_tune_results %>%
  select_best(metric = "recall")

svm_rbf_wf_final <- svm_rbf_wf %>%
  finalize_workflow(best_svm_rbf)

svm_rbf_fit <- svm_rbf_wf_final %>%
  fit(train)
...
svm_rbf_fit |>
  augment(train) |>
  recall(.pred_class, transported) |>
  kable() |>
  kable_styling()

```

We see a recall of 0.68 which is not very good, and is also not close to the results from the hyperparameter tuning. I'm not quite sure what is going on here, and will require some more attention to resolve.

Regardless, let's see how this model does on the test set.
...
As expected, this model did a little bit worse on the test set. This is also quite a bit worse than what we saw from the best logistic regression model, without using hyperparameter tuning. This indicates that we should not trust this support vector machine model to make accurate predictions about our passengers probability of getting transported to an alternate dimension.

## Model Interpretation and Inference

## Conclusion

In this notebook, we have begun exploration into classification models, different model classes, and reasons to use each model class. We explored two binary classifiers, logistic regression and support vector machines, which come in handy when we need to know if an observation belongs to one of two classes. We also explored performance metrics that are useful for classification, such as accuracy, precision, and recall. These metrics will be used throughout this course. 
